...
object NetworkDepthTest extends App with BaseParsers {
  ...
  class Conf(arguments: Seq[String]) extends ScallopConf(arguments) {
    val depth = opt[Int](required = true)
  ...
  private val chainsNo = 100
  private val inputPort = "eth0"
  private val depth = conf.depth()
  ...
  /////////////////////////////////////////
  /// Run the test for the given depth
  /////////////////////////////////////////

  val indices = Random.shuffle((0 until chainsNo).toList).take(depth)
  val drivers = indices.map(i =>
      getDriver(s"ipt$i", getConfFile(conf.chain_size(), i)))
  val iptRouters = drivers.map(_.iptRouter)

  val t0 = System.nanoTime()
  val (successful, failed) = drivers(0).symExec(
    vds = iptRouters,
